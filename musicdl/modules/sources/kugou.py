...
class KugouMusicClient(BaseMusicClient):
    ...
    def _search(self, keyword: str = '', search_url: str = '', request_overrides: dict = None, song_infos: list = [], progress: Progress = None, progress_id: int = 0):
        ...
        try:
            ...
            task_id = progress.add_task(f"{self.source}._search >>> Start to process the 0th search result on page {page_no}", total=self.search_size_per_page if self.strict_limit_search_size_per_page else len(resp2json(resp)['data']['info']), completed=0)
            for search_result_idx, search_result in enumerate(resp2json(resp)['data']['info']):
                # --update progress
